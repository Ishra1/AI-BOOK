...
- [ ] T001 Initialize a new Docusaurus v3.9 project in the repository root.
- [ ] T002 Create the main content directory `docs/`.
- [ ] T003 [P] Create the directory structure for all 13 weeks inside `docs/` (e.g., `docs/01-week-1-foundations/`, `docs/02-week-2-sensing/`, etc.) per `plan.md`.
- [ ] T004 Configure the Docusaurus `docusaurus.config.js` with the book title "Physical AI & Humanoid Robotics" and theme settings.
...
- [ ] T005 [P] Create an `intro.mdx` file in `docs/` with a brief course overview.
- [ ] T006 [P] Create placeholder Markdown files (e.g., `01-intro-to-physical-ai.mdx`) for every module listed in the `plan.md` file structure. Each file should contain a title and "Content to be added."
- [ ] T007 [P] Create the `hardware-requirements.mdx` file in `docs/`.
- [ ] T008 Configure the `sidebars.js` file to create a collapsible navigation menu matching the 13-week structure from `plan.md`.
...
- [ ] T009 [US1] Write the full content for all modules in `docs/01-week-1-foundations/` based on the spec.
- [ ] T010 [US1] Write the full content for `docs/03-week-3-ros/01-intro-to-ros2.mdx`.
- [ ] T011 [P] [US1] Add learning outcomes to the top of each completed content file.
- [ ] T012 [P] [US1] Add APA-style inline citations for technical concepts in the written content.
- [ ] T013 [US1] Verify that the Docusaurus site builds successfully (`npm run build`) and navigation for all 13 weeks works as expected.
...
- [ ] T014 [US2] Add Python/rclpy code examples to the content in `docs/03-week-3-ros/02-nodes-topics-services.mdx`.
- [ ] T015 [P] [US2] Add a URDF example to `docs/05-week-5-simulation/02-urdf-and-xacro.mdx`.
- [ ] T016 [US2] Confirm that code blocks are rendered correctly in the Docusaurus site and that the built-in copy button is functional.
...
- [ ] T017 [US3] Populate the `docs/hardware-requirements.mdx` file with the list of required hardware: RTX GPU, Jetson Orin Nano, RealSense D435i, and Unitree robots.
...
- [ ] T018 [P] Add a placeholder component or section in the Docusaurus layout for a future RAG chatbot UI.
- [ ] T019 [P] Add a placeholder button or link in the UI for a future language-switching (Urdu translation) feature.
- [ ] T020 [P] Review the overall site for mobile responsiveness and note any display issues.
- [ ] T021 [P] Create a placeholder `docs/07-week-7-midterm/midterm-project.mdx` with a description of the assessment.
